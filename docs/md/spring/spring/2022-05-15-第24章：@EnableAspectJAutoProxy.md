...
在`AspectJAutoProxyRegistrar#registerBeanDefinitions()`方法中调用了`AopConfigUtils.registerAspectJAnnotationAutoProxyCreatorIfNecessary(registry)`方法来注册`registry`，
...
AnnotationAwareAspectJAutoProxyCreator 继承的结构图：
